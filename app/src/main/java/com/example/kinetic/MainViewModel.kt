...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val syncRepo = SyncRepository(db, NetworkClient.api, PreferencesManager(application, UserProfileManager(application)))
    ...
    private val _exerciseSummaries = MutableStateFlow<Map<String, ExerciseSummary>>(emptyMap())
    val exerciseSummaries: StateFlow<Map<String, ExerciseSummary>> = _exerciseSummaries
    ...
    private val _todayWorkout = MutableStateFlow<TodayWorkout?>(null)
    val todayWorkout: StateFlow<TodayWorkout?> = _todayWorkout

    private val _debugInfo = MutableStateFlow("Se incarca...")
    ...
    init {
        ...
        // React to account switches: re-seed and reload data for the new user
        viewModelScope.launch {
            CurrentUserProvider.getInstance().currentUserId
                .collect { uid ->
                    loadWorkouts(uid)
                    loadExercises()
                    computeVolumes(uid)
    ...
    fun saveWorkout(userId: String, session: TrainingSession, notes: String = "", durationMs: Long = 0L) {
        viewModelScope.launch {
            antrenamentRepo.saveAntrenament(userId, session, notes, durationMs)
    ...
    fun toggleFavorite(name: String, userId: String = "", group: String = "") {
        viewModelScope.launch {
            antrenamentRepo.toggleFavorite(name, userId, group)
            loadExercises()
        }
    }
    ...
    fun getExercitiiPentruGrupa(userId: String, grupa: String, callback: (List<ExerciseListItem>) -> Unit) {
        viewModelScope.launch {
            val result = antrenamentRepo.getExercitiiPentruGrupaSimple(userId, grupa)
            callback(result)
        }
    }

    fun adaugaExercitiuCustom(userId: String, grupa: String, nume: String, callback: () -> Unit) {
        viewModelScope.launch {
            antrenamentRepo.adaugaExercitiuCustom(userId, grupa, nume)
            callback()
        }
    }

    fun setFavorite(userId: String, grupa: String, numeExercitiu: String, isFavorite: Boolean, callback: () -> Unit) {
        viewModelScope.launch {
            antrenamentRepo.setFavoriteSimple(userId, grupa, numeExercitiu, isFavorite)
            callback()
        }
    }

    fun getFavoriteExercises(userId: String, callback: (Map<String, List<ExerciseListItem>>) -> Unit) {
        viewModelScope.launch {
            val result = antrenamentRepo.getFavoriteExercises(userId)
            callback(result)
        }
    }

    fun salveazaAntrenament(userId: String, grupaMusculara: String, numeExercitiu: String, seturi: List<SetEntry>, note: String, durationMs: Long = 0L, callback: (Boolean) -> Unit) {
        viewModelScope.launch {
            val isNewPR = antrenamentRepo.salveazaAntrenamentSimple(userId, grupaMusculara, numeExercitiu, seturi, note, durationMs)
            callback(isNewPR)
        }
    }

    fun getIstoricExercitiu(userId: String, exerciseName: String, callback: (List<ExercitiuEntity>) -> Unit) {
        viewModelScope.launch {
            val result = antrenamentRepo.getIstoricExercitiu(userId, exerciseName)
            callback(result)
        }
    }

    fun getStatisticiExercitiu(userId: String, exerciseName: String, callback: (ExerciseStats) -> Unit) {
        viewModelScope.launch {
            val result = antrenamentRepo.getStatisticiExercitiu(userId, exerciseName)
            callback(result)
        }
    }
    ...
    fun getRecuperareMusculara(userId: String, grupa: String, callback: (Double) -> Unit) {
        viewModelScope.launch {
            val result = antrenamentRepo.getRecuperareMusculara(userId, grupa)
            callback(result)
        }
    }

    fun getToateRecuperarile(userId: String, callback: (List<Pair<String, Double>>) -> Unit) {
        viewModelScope.launch {
            val result = antrenamentRepo.getToateRecuperarile(userId)
            callback(result)
        }
    }

    fun incarcaUltimulAntrenament(userId: String, exerciseName: String, callback: (List<SetEntry>) -> Unit) {
        viewModelScope.launch {
            val result = antrenamentRepo.incarcaUltimulAntrenament(userId, exerciseName)
            callback(result)
        }
    }
    ...
    fun getProgresLunar(userId: String, exerciseName: String, callback: (List<ProgresLunar>) -> Unit) {
        viewModelScope.launch {
            val result = antrenamentRepo.getProgresLunar(userId, exerciseName)
            callback(result)
        }
    }

    fun loadExerciseSummaries(userId: String, exerciseNames: List<String>) {
        viewModelScope.launch {
            val result = antrenamentRepo.getExerciseSummaries(userId, exerciseNames)
            _exerciseSummaries.value = _exerciseSummaries.value + result
        }
    }
    ...
    fun computeTodayWorkout(preferencesManager: PreferencesManager) {
        ...
        val deloadFactor = preferencesManager.getDeloadReductionFactor()
        val workout = WorkoutCycleGenerator.buildTodayWorkout(profile, startDate, isDeloadActive, deloadFactor)

        val debug = buildString {
            appendLine("=== DEBUG ===")
            appendLine("azi: ${LocalDate.now()}")
            appendLine("startDate: $startDate")
            appendLine("zile/sapt: ${profile.sessionsPerWeek}")
            appendLine("grupe: ${profile.selectedGroups}")
            appendLine("onboarding: ${preferencesManager.isOnboardingComplete()}")
            appendLine("dayInCycle: ${workout.dayInCycle}")
            appendLine("dayType: ${workout.dayType}")
            appendLine("muscleGroups: ${workout.muscleGroups}")
            appendLine("exercises: ${workout.exercises.size}")
            appendLine("deloadActive: $isDeloadActive")
        }
        _debugInfo.value = debug
        _todayWorkout.value = workout
    }

    fun refreshTodayWorkout(preferencesManager: PreferencesManager) {
        computeTodayWorkout(preferencesManager)
    }
